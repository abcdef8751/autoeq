...
const optimisation_threads_count: usize = 2;
...
const input_file: &str = "./measurements/Buds2Pro ANC off avg.txt";
const output_file: &str = "./measurements/Pilgrim L (CRIN).txt";

fn parse_measurement_file(file_path: &str) -> Vec<Point> {
    let bytes = fs::read(file_path).unwrap();
    let file = unsafe { str::from_utf8_unchecked(&bytes) }; //.unwrap();
    file.lines()
        .map(|x| {
...
fn random(maximum: f64) -> f64 {
    (rand::random::<f64>() * 2f64 - 1f64) * maximum
}
fn optimise(
    ...
    for iter in 0..iter_count {
        ...
        for filter in filters.clone() {
            ...
            for i in 0..thread_count {
                ...
                let measurement_dif = measurement_dif.clone();
                let thread = thread::spawn(move || {
                    ...
                    (error(&filters, &measurement_dif), filters)
                });
                handles.push(thread);
            }
            let (new_error, new_filters) = handles
                .into_iter()
                .map(|x| x.join().unwrap())
                //                .collect::<Vec<(f64, Vec<Filter>)>>()
...
fn main() {
    ...
    for i in 0..optimisation_rounds {
        ...
        let mut handles = vec![];
        for _ in 0..optimisation_threads_count {
    ...
    let eq_file_content = format!(
        ...
        best_filters
            ...
            .map(|(i, x)| format!(
                ...
                i + 1,
                x.center.round(),
                (x.gain * 100.0).round() / 100.0,
                (x.Q * 100.0).round() / 100.0
            ))
            .collect::<Vec<_>>()
            .join("\n")
    );
    println!("{}", eq_file_content);
    let _ = fs::create_dir("./eq");
    fs::write(
        format!(
            "./eq/eq_{}_{}.txt",
    ...
    .unwrap();
    println!(
        "{} iterations by {} threads and {} optimisation threads for {} rounds took: {:?}",
        iters,
        thread_count,
        optimisation_threads_count,
        optimisation_rounds,
        start_timer.elapsed()
    );
}
